-- @description CSB (Horns, Trombones, Tuba) Auto Legato
-- @author Harry Brokensha
-- @version 0.1
-- @changelog
--  + init

-- hb_Apply Cinematic Studio Brass (Tbns, Horns, Tuba) legato transitions (based on note velocity, leaving note-on of first note and note-off of last note)
...
-- CSB (Trombones, Horns, Tuba) legato transitions offsets:----
med_offset = -230 --ms
fast_offset = -100 --ms
-------------------------------------------------
...
function NOTES()
  ...
  for i = 0, notes - 1 do
    ...
    if selected == true and i ~= 0 and i ~= first_selected_note then    
      ...
      if (vel >= 0 and vel <= 64) then -- medium
...
function main()
  ...
  reaper.Undo_EndBlock("CSB (Tbns, Horns, Tuba) Legato transitions: Move events", -1)
