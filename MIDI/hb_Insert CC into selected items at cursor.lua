-- @description Insert CC into selected items at cursor
-- @author Harry Brokensha
-- @version 1.0
-- @changelog
--  + init

-- hb_Insert CC into selected items at cursor
...
function getUserValue()
  user_ok, userValue = reaper.GetUserInputs("Set value", 1, "1-127", "64")
  if not user_ok then return end
  return userValue
end

function getUserCCNumber()
  user_ok, userCC = reaper.GetUserInputs("Set CC", 1, "1-127", "1")
  if not user_ok then return end
  return userCC
end
...
function main()
  channel = 0
  user_ok, userChannel = reaper.GetUserInputs("Set MIDI Channel (17 for all channels)", 1, "1-17", "1")
  if not user_ok then return end

  if userChannel == "17" then
    allChannels = true
  end

  userValue = getUserValue()

  userCC = getUserCCNumber()
  ...
  for itemIndex = 0, numItems - 1 do
    ...
    userChannel = tonumber(userChannel)
    if userChannel == nil or userChannel < 1 or userChannel > 16 then 
      channel = 1
    end
