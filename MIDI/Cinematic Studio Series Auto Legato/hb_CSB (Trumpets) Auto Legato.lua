-- @description CSB (Trumpets) Auto Legato
-- @author Harry Brokensha
-- @version 0.1
-- @changelog
--  + init

-- hb_Apply Cinematic Studio Brass (Tpts) legato transitions (based on note velocity, leaving note-on of first note and note-off of last note)
...
-- CSB (Tpts) legato transitions offsets:----
med_offset = -180 --ms
fast_offset = -100 --ms
-------------------------------------------------
...
function main()
  ...
  reaper.Undo_EndBlock("CSB (Tpts) Legato transitions: Move events", -1)
