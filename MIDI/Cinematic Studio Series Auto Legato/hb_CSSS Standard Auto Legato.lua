-- @description CSSS Standard Auto Legato
-- @author Harry Brokensha
-- @version 0.1
-- @changelog
--  + init

-- hb_Apply Cinematic Studio Solo Strings Standard legato transitions (based on note velocity, leaving note-on of first note and note-off of last note)
...
-- CSSS Standard legato transitions offsets:----
med_offset = -250 --ms
fast_offset = -100 --ms
-------------------------------------------------
...
function main()
  ...
  reaper.Undo_EndBlock("CSSS Standard Legato transitions: Move events", -1)
